...
int readserverent(FILE *fd, struct server *serverdata)
{
	...
	for (;;) {
		...
		if ((sscanf(line, "\"%[^\"]\" %s %d", serverdata->name, serverdata->host,
			&(serverdata->port))!=3) &&
			(sscanf(line, "%s %s %d", serverdata->name, serverdata->host,&(serverdata->port))!=3))
		{
			serverdata->port = DEFAULT_PORT;
			if ((sscanf(line, "\"%[^\"]\" %s", serverdata->name, serverdata->host)!=2) &&
				(sscanf(line, "%s %s", serverdata->name, serverdata->host)!=2))
			{
...
void listservers()
{
	...
	while (readserverent(serverfile, &serverbuf)) {
		printf("%-16s%-32s%5d\n", serverbuf.name, serverbuf.host, serverbuf.port);
	}
	fclose(serverfile);
}
