...
static cmd_handler_t c_cmd_handlers[] = {
    {"c_color", c_color},
    {"c_hclear", c_hclear},
    {"c_hdel", c_hdel},
    {"c_help", c_help},
...
static char **
split_args(const char *args_str, int *argc_out)
{
    char **argv = NULL;
    int argc = 0;
    int max_args = 32;
...
static int
tcl_has_handler(const char *cmd)
{
    if (interp == NULL)
        return 0;
...
static int
call_tcl_handler(const char *cmd, const char *args_str)
{
    if (interp == NULL)
        return TCL_ERROR;
    ...
    if (!Tcl_GetCommandInfo(interp, cmd, &cmdinfo)) {
        ...
        char eval_cmd[256];
        snprintf(eval_cmd, sizeof(eval_cmd), 
                 "set matches [info commands %s*]; if {[llength $matches] == 1} {lindex $matches 0} else {}", cmd);
